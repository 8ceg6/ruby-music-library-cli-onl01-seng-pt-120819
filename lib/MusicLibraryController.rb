class MusicLibraryController 
  ...
  def initialize(path= "./db/mp3s")
      imported_file = MusicImporter.new(path)
  ...
  def list_artists
      ...
      artists_list.each.with_index(1) do |artist, index|
        puts "#{index}. #{artist.name}"
      end 
  end 
  
  def list_genres 
      genre_sort = Genre.all.sort_by {|genre| genre.name}
      genre_sort.each.with_index(1) do |genre,index|
